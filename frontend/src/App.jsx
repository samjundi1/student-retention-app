...
function App() {
  // Store values as strings initially to allow typing decimals (e.g., "3.")
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Server Error (${response.status}): ${errorText || "Check backend logs"}`);
      }
      ...
      console.error(err);
      setError(`Connection Failed: Is the backend running at ${apiUrl}?`);
    } finally {
      setLoading(false);
    }
  };

  // ADDED 'text-gray-900' to className below
  const InputField = ({ label, name, step = "0.1" }) => (
    <div className="flex flex-col">
      ...
      <input
        ...
        className="p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:outline-none transition bg-white text-gray-900" 
        required
      />
    </div>
  );
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-gray-100">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-3/5 p-8 overflow-y-auto max-h-[90vh]">
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-gray-800">Student Retention AI</h1>
            <p className="text-gray-500 text-sm mt-1">Predict student persistence based on academic & demographic data.</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <h3 className="text-gray-700 font-bold border-b pb-1">Academic Performance</h3>
            ...
            <h3 className="text-gray-700 font-bold border-b pb-1 pt-2">Program Details</h3>
            <div className="grid grid-cols-2 gap-4">
            ...
            <h3 className="text-gray-700 font-bold border-b pb-1 pt-2">Demographics</h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              <SelectField label="Funding" name="Funding" options={[
                { value: 2, label: "GPOG_FT" }, { value: 4, label: "Intl Regular" }, { value: 1, label: "Apprentice" }
              ]} />
            ...
            <button 
              type="submit" 
              disabled={loading}
              className={`w-full py-3 mt-6 rounded-lg font-bold text-white transition-all transform hover:scale-[1.02] ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 shadow-lg'}`}
            >
        ...
        <div className="w-full md:w-2/5 bg-gray-50 border-l border-gray-100 p-8 flex flex-col justify-center items-center text-center">
          ...
          {result && (
            <div className="w-full animate-fade-in">
              <div className={`inline-flex items-center justify-center w-24 h-24 rounded-full mb-6 shadow-inner ${result.prediction === 1 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                <span className="text-4xl font-bold">{Math.round(result.probability * 100)}%</span>
              </div>
              ...
              <p className="text-gray-500 mb-8">
                {result.prediction === 1 
                  ? "Student is likely to continue their studies." 
